...
def diagramma1(workouts):
    ...
    for w in workouts:
        ...
        workouts_stat[w['type']].append(w)
    labels = []
    sizes = []  # значения для каждого сектора
    colors = ['red', 'green', 'orange', 'yellow', 'blue']
    explode = (0, 0, 0, 0, 0)  # "вынесем" второй сектор для акцента
    for key in workouts_stat:
        a = workouts_stat[key]
        labels.append(key)
        sizes.append(len(a)/len(workouts))

    # Построение диаграммы
    plt.pie(sizes, labels=labels, colors=colors, explode=explode,
            autopct='%1.1f%%', shadow=True, startangle=90)
    plt.title("Круговая диаграмма фруктов")
    plt.axis('equal')  # чтобы круг был ровным
    plt.show()

def diagramma2(users):
    # Данные
    categories = []
    values = []
    for u in users:
        categories.append(u['name'])
        values.append(len(u['workouts']))
...
def diagramma3(workouts):
    ...
    plt.title("Активность тренировок(калорий в минуту) ")
    plt.xlabel("Тип тренировки")
    plt.ylabel("Калорий в минуту")
    plt.xticks(rotation=45)
    # Добавляем значения над столбцами
    for bar in bars:
        height = bar.get_height()
